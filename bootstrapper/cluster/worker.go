...
provisioning:
  ...
  maxSize: {{.Persistent}}GiB
`

func (c Config) generateWorkerYAML(outPath string, worker NodeConfig, controlPlaneAddress string) error {
	...
	data := map[string]any{
		...
		"InstallDisk":         worker.StorageType.InstallDisk(),
