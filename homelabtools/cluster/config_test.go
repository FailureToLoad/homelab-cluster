...
func TestGenerateConfigs(t *testing.T) {
	...
	t.Run("generates control plane config successfully", func(t *testing.T) {
		...
		assert.Equal(t, "192.168.1.100:6443", controlPlane["endpoint"])
	...
	t.Run("generates talosconfig file", func(t *testing.T) {
		...
		assert.Equal(t, "dGVzdC1vcy1jZXJ0", clusterContext["crt"])
		assert.Equal(t, "dGVzdC1vcy1rZXk=", clusterContext["key"])
	})
}
